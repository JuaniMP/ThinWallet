...
export const circleService = {
  ...
  getCirclesAsMember: async (idUsuario: number) => {
    return api.get<CirculoGasto[]>(`${CIRCULOS_GASTO_BASE}/miembro/${idUsuario}`);
  },

  getCircleDetail: async (idCirculoGasto: number) => {
    return api.get<CirculoDetalle>(`${CIRCULOS_GASTO_BASE}/${idCirculoGasto}/detalle`);
  },
  ...
  getAllTipoCirculos: async () => {
    try {
      ...
      console.warn(`El endpoint ${TIPOS_CIRCULO_BASE} devolvió una lista vacía`);
    } catch (error) {
      console.warn(`No se pudo cargar tipos de círculo desde ${TIPOS_CIRCULO_BASE}`, error);
    }
