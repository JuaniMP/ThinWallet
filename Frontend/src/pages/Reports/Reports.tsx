...
export function Reports() {
  ...
  const categoryData = useMemo(() => {
    const map: Record<string, { nombre: string; income: number; expense: number }> = {};
    transactions.forEach(t => {
      const key = t.categoryId || '0';
      const cat = categories.find(c => String(c.idCategoria) === key);
      const nombre = cat?.nombre ?? (key === '0' ? 'Sin Categoría' : `Categoría ${key}`);
  ...
  const maxCat = Math.max(...(categoryData.length > 0 ? categoryData.map(c => c.total) : [1]));
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="reports-summary">
              ...
              <div className={`report-card neo-shadow ${metrics.balance >= 0 ? 'rc-balance-pos' : 'rc-balance-neg'}`}>
                    ...
                    <div className="chart-column">
                      <div className="chart-bar-wrap">
                        <div
                          ...
                          style={{ height: `${(metrics.income / maxBar) * 180}px` }}
                        />
                      </div>
                      <p className="chart-label">Ingresos</p>
                      <p className="chart-sublabel income">{fmt(metrics.income)}</p>
                    </div>
                    <div className="chart-column">
                      <div className="chart-bar-wrap">
                        <div
                          className="chart-bar expense-bar"
                          style={{ height: `${(metrics.expense / maxBar) * 180}px` }}
                        />
                      </div>
                      <p className="chart-label">Gastos</p>
                      <p className="chart-sublabel expense">{fmt(metrics.expense)}</p>
                    </div>
                  </div>
                </div>
              )}
                        ...
                        {cat.income > 0 && (
                          <div className="cat-bar-line">
                            <div
                              ...
                              style={{ width: `${(cat.income / maxCat) * 100}%` }}
                            />
                            <span className="cat-bar-amt income">{fmt(cat.income)}</span>
                          </div>
                        )}
                        {cat.expense > 0 && (
                          <div className="cat-bar-line">
                            <div
                              ...
                              style={{ width: `${(cat.expense / maxCat) * 100}%` }}
                            />
                            <span className="cat-bar-amt expense">{fmt(cat.expense)}</span>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="payment-track">
                    {paymentData.cash > 0 && (
                      <div
                        ...
                        style={{ width: `${(paymentData.cash / paymentData.total) * 100}%` }}
                      />
                    )}
                    {paymentData.card > 0 && (
                      <div
                        className="payment-seg card-seg"
                        style={{ width: `${(paymentData.card / paymentData.total) * 100}%` }}
                      />
                    )}
                  </div>
                  <div className="payment-legend">
                    <div className="payment-item">
                      ...
                      <div>
                        ...
                        <p className="pm-value">{paymentData.cash} transacciones</p>
                      </div>
                      <p className="pm-pct">
                        {paymentData.total > 0
                          ? Math.round((paymentData.cash / paymentData.total) * 100)
                          : 0}%
                      </p>
                    </div>
                    <div className="payment-item">
                      ...
                      <div>
                        ...
                        <p className="pm-value">{paymentData.card} transacciones</p>
                      </div>
                      <p className="pm-pct">
                        {paymentData.total > 0
                          ? Math.round((paymentData.card / paymentData.total) * 100)
                          : 0}%
                      </p>
                    </div>
                  </div>
