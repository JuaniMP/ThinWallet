...
export function CircleDetail() {
  ...
  const totalIngresos = useMemo(
    () =>
      transacciones
        .filter(esIngresoPor)
        .reduce((s, t) => s + (Number(t.montoOriginal) || 0), 0),
    [transacciones], // eslint-disable-line react-hooks/exhaustive-deps
  );
  ...
  const formatMoney = (amount: number) =>
    amount.toLocaleString("es-CO", {
      style: "currency",
      currency: detail?.monedaBase || "COP",
  ...
  const resolverTipoMovimiento = (): string => {
    ...
    if (gastoEsIngreso) {
      const match = tiposMovimiento.find((t) =>
        DEPOSITO_KEYS.some((k) => t.nombre.toUpperCase().includes(k)),
      );
      return match?.nombre ?? tiposMovimiento[0]?.nombre ?? "DEPOSITO";
    }
    const match = tiposMovimiento.find((t) =>
      RETIRO_KEYS.some((k) => t.nombre.toUpperCase().includes(k)),
    );
    return (
      match?.nombre ??
      tiposMovimiento[1]?.nombre ??
      tiposMovimiento[0]?.nombre ??
      "RETIRO"
    );
  };
  ...
  const handleCrearTransaccion = async () => {
    ...
    if (isNaN(monto) || monto <= 0) {
      setGastoError("El monto debe ser un número positivo.");
      return;
    }
  ...
  const handleEliminarTransaccion = async (idTransaccion: number) => {
    ...
    try {
      ...
      setTransacciones((prev) =>
        prev.filter((t) => t.idTransaccion !== idTransaccion),
      );
    } catch {
      // silently ignore
    }
  ...
  const handleSearchChange = useCallback((q: string) => {
    ...
    searchDebounce.current = setTimeout(async () => {
      ...
      try {
        const results = await usuarioService.buscarUsuarios(
          q.trim(),
          user?.idUsuario,
        );
  ...
  const handleInvite = async (invitado: UsuarioBusqueda) => {
    ...
    setInviteSuccess("");
    try {
      const updated = await circleService.inviteRegisteredUser(
        detail.idCirculoGasto,
        invitado.idUsuario,
      );
      setDetail(updated);
      setSearchQuery("");
  ...
  const isCreator =
    user && detail && detail.idUsuarioCreador === user.idUsuario;
          ...
          <>
            ...
            <section className="circle-metrics-row">
              ...
              <div className="circle-metric-card neo-shadow">
                ...
                <strong
                  className="metric-value"
                  style={{ color: "#e53e3e", fontSize: "0.95rem" }}
                >
              ...
              <div className="circle-metric-card neo-shadow">
                ...
                <strong
                  className="metric-value"
                  style={{
                    color: balanceTotal >= 0 ? "var(--primary)" : "#e53e3e",
                    fontSize: "0.95rem",
                  }}
                >
                  {balanceTotal >= 0 ? "+" : ""}
            ...
            <section className="circle-panel neo-shadow">
              ...
              <div className="attrs-grid">
                ...
                <div>
                  <span>Moneda</span>
                  <strong>{detail.monedaBase || "COP"}</strong>
                </div>
                <div>
                  <span>Estado</span>
                  <strong>
                    {detail.estado === 1 || detail.estado === "ACTIVO"
                      ? "Activo"
                      : "Inactivo"}
                  </strong>
                </div>
            ...
            <section className="circle-actions-panel neo-shadow">
              <div className="balance-box">
                ...
                <strong
                  ...
                  style={{ color: balanceTotal >= 0 ? undefined : "#e53e3e" }}
                >
                  {formatMoney(balanceTotal)}
                </strong>
              </div>
            ...
            <section className="circle-history-panel neo-shadow">
              ...
              {transacciones.length === 0 ? (
                <div className="history-empty">
                  <p>No hay transacciones en este círculo aún.</p>
                </div>
              ) : (
                <div className="history-timeline">
                  {transacciones.map((t) => {
                    ...
                    return (
                      <div
                        key={t.idTransaccion}
                        className={`history-item history-${esIngreso ? "income" : "expense"}`}
                      >
                        ...
                        >
                          ...
                          >
                            <strong
                              ...
                              style={{
                                color: esIngreso ? "var(--primary)" : "#e53e3e",
                              }}
                            >
                              {esIngreso ? "+" : "-"}
                              {formatMoney(Number(t.montoOriginal))}
                            </strong>
                            {isCreator && (
                              <button
                                type="button"
                                onClick={() =>
                                  void handleEliminarTransaccion(
                                    t.idTransaccion,
                                  )
                                }
                                style={{
            ...
            <section className="circle-panel neo-shadow">
              ...
              <div className="guest-token-grid">
                ...
                >
                  ...
                  {detail.correoCreador &&
                    !detail.correoCreador.includes("@thinwallet.local") && (
                      <p
                ...
                {detail.invitados.length === 0 ? (
                  <p
                    ...
                    style={{ padding: "12px 0", gridColumn: "1/-1" }}
                  >
                  ...
                  detail.invitados.map((m) => (
                    <article key={m.idUsuario} className="guest-token-item">
                      ...
                      {m.tipoUsuario === "FANTASMA" &&
                        m.tokenInvitacionPersonal && (
                          <div
                            style={{
                              marginTop: "6px",
                              display: "flex",
                          ...
                          >
                            <input
                              type="text"
                              readOnly
                              value={m.tokenInvitacionPersonal}
                              style={{
                                flex: 1,
                                padding: "4px 6px",
            ...
            {isCreator && (
              <section className="circle-panel neo-shadow">
                <div className="panel-header">
                  <h3>Invitar usuario registrado</h3>
                </div>
            ...
            <div className="modal-content neo-shadow">
              ...
              <div className="modal-body">
                ...
                <div className="input-group">
                  ...
                  <input
                    value={gastoContexto}
                    onChange={(e) => setGastoContexto(e.target.value)}
                    placeholder="Ej: Viaje, Cena, Mercado..."
                  />
                </div>
                {gastoError && (
                  <p
